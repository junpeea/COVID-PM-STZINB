...
zinb_county_corona <- function (state, adj.mat, aggregate.state, positives.county, cumulative,
  ...
  x0 <- rep(scale(aggregate.state$population), each=dim(positives.county)[2]) # Population size as offset
  ...
  # x3 <- rep(scale(aggregate.state$popdensity), each=dim(positives.county)[2])
  ...
  library(splines)
  # Bmatrix = bs(tt,knots=c(0.25,0.50,0.75)*n.dates,Boundary.knots=c(0,1),degree=3,intercept=TRUE)
  ...
  X <- cbind(1, tt, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x0, Bmatrix); p<-ncol(X)
  ...
  for (i in 1:nsim){
    ...
    if (offset) {
      # X <- cbind(1, tt, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x0, Bmatrix); p<-ncol(X)
      # X <- cbind(1, tt, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x0, Bmatrix); p<-ncol(X)
      alpha[2]   <- 0 #Nonlinear time trend
      alpha[17]  <- 1 #offset
    }
    ...
    if (offset) {
      # X <- cbind(1, tt, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x0, Bmatrix); p<-ncol(X)
      # X <- cbind(1, tt, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x0, Bmatrix); p<-ncol(X)
      beta[18:24]  <- 0 #Linear time trend
      beta[17] <- 1 #offset
    }
