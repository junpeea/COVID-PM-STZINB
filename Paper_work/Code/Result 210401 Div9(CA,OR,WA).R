...
state <- "California"
state.abbr1 <- "CA"
mydat.CA <- generate_data(state = state,
                          state.abbr = state.abbr1,
                          start.date = start.date,
                          end.date = end.date)
dim(mydat.CA$aggregate.state) # [1] 58 98

state <- "Oregon"
state.abbr2 <- "OR"
mydat.OR <- generate_data(state = state,
                          state.abbr = state.abbr2,
                          start.date = start.date,
                          end.date = end.date)
dim(mydat.OR$aggregate.state) # [1] 36 98

state <- "Washington"
state.abbr3 <- "WA"
mydat.WA <- generate_data(state = state,
                          state.abbr = state.abbr3,
                          start.date = start.date,
                          end.date = end.date)
dim(mydat.WA$aggregate.state) # [1] 39 98


# Combine datasets under Division 9
mydat.div9 = list()
mydat.div9$deaths.county = rbind(mydat.CA$deaths.county,mydat.OR$deaths.county,mydat.WA$deaths.county)
mydat.div9$cum.deaths.county = rbind(mydat.CA$cum.deaths.county,mydat.OR$cum.deaths.county,mydat.WA$cum.deaths.county)
mydat.div9$aggregate.state   = rbind(mydat.CA$aggregate.state,mydat.OR$aggregate.state,mydat.WA$aggregate.state)
...
idx <- which(county.list %in% c(get.county("CA"),get.county("OR"),get.county("WA")))
mydat.div9$adj.mat <-  county.adj.mat[idx,idx]
...
zinb.daily.offset$Beta[,11] %>% plot(type='l')
zinb.daily.offset$Beta %>% apply(2,function(w) quantile(w,probs=c(0,0.025,0.5,0.975,1)))
zinb.daily.offset$Beta[6000:8000,] %>% apply(2,function(w) quantile(w,probs=c(0,0.025,0.5,0.975,1)))
