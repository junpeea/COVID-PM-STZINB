...
zinb_county_corona <- function (state, adj.mat, aggregate.state, positives.county, cumulative,
  ...
  phi_init<-spam::rmvnorm(1,Sigma=diag(rep(c(.1, .1, 5, 5),times=n)))	  # Random effects
  ...
  for (i in 1:nsim){
    ...
    # Update alpha
    mu<-X%*%alpha+x0+Phi1+Phi2*tt
    w<-rpg(N,1,mu)
    z<-(y1-1/2)/w           
    v<-solve(crossprod(sqrt(w)*X)+T0a)  
    m<-v%*%(T0a%*%alpha0+t(sqrt(w)*X)%*%(sqrt(w)*(z-x0-Phi1-Phi2*tt)))
    alpha<-c(rmvnorm(1,m,v))
    ...
    # prec<-solve(priorprec)+as.spam(diag(tapply(w,id,sum),n,n))
    # m<-c(priorprec%*%priormean)+tapply(w*(z-X%*%alpha-Phi2*tt),id,sum)
    m<-c(solve(prec)%*%(solve(priorprec)%*%(priormean)))+tapply(w*(z-X%*%alpha-x0-Phi2*tt),id,sum)
    ...
    # prec<-solve(priorprec)+as.spam(diag(tapply(w*tt^2,id,sum),n,n))	
    # m<-c(priorprec%*%priormean)+tapply(tt*w*(z-X%*%alpha-Phi1),id,sum)
    m<-c(solve(prec)%*%(solve(priorprec)%*%(priormean)))+tapply(tt*w*(z-X%*%alpha-x0-Phi1),id,sum)
    ...
    eta1<-X%*%alpha+x0+Phi1+Phi2*tt
    eta2<-X%*%beta+x0+Phi3+Phi4*tt              # Use all n observations
    ...
    ratio<-sum(dnbinom(y[y1==1],rnew,1-q[y1==1],log=T), na.rm = TRUE)-sum(dnbinom(y[y1==1],r,1-q[y1==1],log=T), na.rm=TRUE)+    
      dtnorm(r,rnew,sqrt(s),0,log=T) - dtnorm(rnew,r,sqrt(s),0,log=T)   # Uniform Prior for R 
    ...
    # Update beta
    eta<-X[y1==1,]%*%beta+x0[y1==1]+Phi3[y1==1]+Phi4[y1==1]*tt[y1==1]
    w<-rpg(N1,y[y1==1]+r,eta)                               # Polya weights
    z<-(y[y1==1]-r)/(2*w)                                      
    v<-solve(crossprod(X[y1==1,]*sqrt(w))+T0b)               
    m<-v%*%(T0b%*%beta0+t(sqrt(w)*X[y1==1,])%*%(sqrt(w)*(z-x0[y1==1]-Phi3[y1==1]-Phi4[y1==1]*tt[y1==1])))  
    beta<-c(rmvnorm(1,m,v))
    ...
    # prec<-solve(priorprec)+as.spam(diag(tapply(w,id[y1==1],sum),n,n))
    ...
    tmp[nis1>0]<-tapply(w*(z-array(X[y1==1,]%*%beta)-x0[y1==1]-Phi4[y1==1]*tt[y1==1]),id[y1==1],sum)
    m<-c(solve(prec)%*%c(solve(priorprec)%*%(priormean)))+tmp
    ...
    # prec<-solve(priorprec)+as.spam(diag(tapply(w*tt[y1==1]^2,id[y1==1],sum),n,n))
    tmp<-rep(0,n)                       # Account for empty counties
    # tmp[nis1>0]<-tapply(tt[y1==1]*w*(z-array(X[y1==1,]%*%beta)-Phi3[y1==1]),id[y1==1],sum)
    # m<-c(priorprec%*%priormean)+tmp
    tmp[nis1>0]<-tapply(tt[y1==1]*w*(z-array(X[y1==1,]%*%beta)-x0[y1==1]-Phi3[y1==1]),id[y1==1],sum)
    m<-c(solve(prec)%*%c(solve(priorprec)%*%(priormean)))+tmp
