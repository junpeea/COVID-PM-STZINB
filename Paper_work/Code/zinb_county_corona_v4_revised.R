...
zinb_county_corona <- function (state, adj.mat, aggregate.state, positives.county, cumulative,
  ...
  library(splines)
  # X <- cbind(1, x1, x2, x32, x33, x34, x4, x5, x6, x7, x8, x9, x10, x12, x13, x14, x0, Bmatrix); p<-ncol(X)
  # X <- cbind(Bmatrix, x0, x1, x2, x32, x33, x34, x4, x5, x6, x7, x8, x9, x10, x12, x14); p<-ncol(X)
  X <- cbind(1, Bmatrix, x0, x1, x2, x32, x33, x34, x4, x5, x6, x7, x8, x9, x10, x12, x14); p<-ncol(X)
  ...
  for (i in 1:nsim){
    ...
    if (offset) {
      ...
      # X <- cbind(1, Bmatrix, x0, x1, x2, x32, x33, x34, x4, x5, x6, x7, x8, x9, x10, x12, x14); p<-ncol(X)
      alpha[(dim(Bmatrix)[2]+2)]  <- 1 #offset
    }
  ...
  mbeta <- apply(res.Beta,2,function(w) quantile(w,probs=c(0,0.025,0.5,0.975,1)))
  beta=round(mbeta[3,], 3)
  bounds.beta <- round(mbeta[c(2,4),], 2)
...
# temp.dat = params$Beta
# colnames(temp.dat) = paste0("c",c(1:ncol(temp.dat)))
# mcmc.temp1.beta = list()
# mcmc.temp1.beta[[1]] = temp.dat[((chain.seed[1]+1):(chain.seed[1]+burn)),c(1,3,4,5)] %>% mcmc
# mcmc.temp1.beta[[2]] = temp.dat[((chain.seed[2]+1):(chain.seed[2]+burn)),c(1,3,4,5)] %>% mcmc
# mcmc.temp1.beta[[3]] = temp.dat[((chain.seed[3]+1):(chain.seed[3]+burn)),c(1,3,4,5)] %>% mcmc
# mcmc.temp1.beta = mcmc.temp1.beta %>% mcmc.list
# pdf(paste0(Sys.Date(),"_",state,"_BetaTraceplot1.pdf"));mcmc_trace(mcmc.temp1.beta);dev.off()
# 
# mcmc.temp2.beta = list()
# mcmc.temp2.beta[[1]] = temp.dat[((chain.seed[1]+1):(chain.seed[1]+burn)),c(6,7,8,9,10,11)] %>% mcmc
# mcmc.temp2.beta[[2]] = temp.dat[((chain.seed[2]+1):(chain.seed[2]+burn)),c(6,7,8,9,10,11)] %>% mcmc
# mcmc.temp2.beta[[3]] = temp.dat[((chain.seed[3]+1):(chain.seed[3]+burn)),c(6,7,8,9,10,11)] %>% mcmc
# mcmc.temp2.beta = mcmc.temp2.beta %>% mcmc.list
# pdf(paste0(Sys.Date(),"_",state,"_BetaTraceplot2.pdf"));mcmc_trace(mcmc.temp2.beta);dev.off()
# 
# mcmc.temp3.beta = list()
# mcmc.temp3.beta[[1]] = temp.dat[((chain.seed[1]+1):(chain.seed[1]+burn)),c(12,14,15,16)] %>% mcmc
# mcmc.temp3.beta[[2]] = temp.dat[((chain.seed[2]+1):(chain.seed[2]+burn)),c(12,14,15,16)] %>% mcmc
# mcmc.temp3.beta[[3]] = temp.dat[((chain.seed[3]+1):(chain.seed[3]+burn)),c(12,14,15,16)] %>% mcmc
# mcmc.temp3.beta = mcmc.temp3.beta %>% mcmc.list
# pdf(paste0(Sys.Date(),"_",state,"_BetaTraceplot3.pdf"));mcmc_trace(mcmc.temp3.beta);dev.off()
# 
# mcmc.temp4.beta = list()
# mcmc.temp4.beta[[1]] = temp.dat[((chain.seed[1]+1):(chain.seed[1]+burn)),c(17,18,19,20)] %>% mcmc
# mcmc.temp4.beta[[2]] = temp.dat[((chain.seed[2]+1):(chain.seed[2]+burn)),c(17,18,19,20)] %>% mcmc
# mcmc.temp4.beta[[3]] = temp.dat[((chain.seed[3]+1):(chain.seed[3]+burn)),c(17,18,19,20)] %>% mcmc
# mcmc.temp4.beta = mcmc.temp4.beta %>% mcmc.list
# pdf(paste0(Sys.Date(),"_",state,"_BetaTraceplot4.pdf"));mcmc_trace(mcmc.temp4.beta);dev.off()
