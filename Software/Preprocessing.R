...
date.base = today("EST")-1
ini <-TRUE
for(dd in 1:Daywindow){
  
  date_of_study = date.base - dd
  # load(paste0("C:/Users/user/Dropbox/Supplements/suppl_",date_of_study,".Rdata"))
  date_of_study = paste0(substr(date_of_study,6,7),"-",substr(date_of_study,9,10),"-",substr(date_of_study,1,4))
  ...
  # state_test = subset(state_test, date ==paste0(substr(date_of_study,7,10),substr(date_of_study,1,2),substr(date_of_study,4,5)))[,-20]
  ...
  Day_out = aggregate_pm_census_cdc_test_beds %>% select(Province_State,Admin2,Confirmed,Deaths,hospitalizedCurrently)
  # Day_out = aggregate_pm_census_cdc_test_beds %>% select(Province_State,Admin2,Confirmed,Deaths,beds)
  temp.date             = aggregate_pm_census_cdc_test_beds$date
  Day_out$date          = as.Date(paste0(substr(temp.date,1,4),"-",substr(temp.date,5,6),"-",substr(temp.date,7,8)))
  Day_out$state         = Day_out$Province_State
  Day_out$county        = Day_out$Admin2
  Day_out$positive      = sapply(log(Day_out$Confirmed+1e-10),function(w) max(w,0)) 
  Day_out$death         = sapply(log(Day_out$Deaths+1e-10),function(w) max(w,0))  
  Day_out$hospitalized  = sapply(log(Day_out$hospitalizedCurrently+1e-10),function(w) max(w,0))
  # Day_out$hospitalized  = sapply(log(Day_out$beds+1e-10),function(w) max(w,0))
  Day_out$hospitalized[is.na(Day_out$hospitalized)] <- 0
  ...
  if(ini==TRUE){
    ...
    All_out = rbind(All_out,Day_out)
  }
  
  
}

summary(All_out);dim(All_out)

write.csv(All_out,file="All3.csv")
